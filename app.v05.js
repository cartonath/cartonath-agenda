...
async function saveAppointment(e){e.preventDefault();const id=appointmentId.value||uid(),old=id?await one(STORES.appointments,id):null,method=await one(STORES.methods,appointmentMethod.value),apps=await all(STORES.appointments);if(C.conflictCount(apps,appointmentDate.value,appointmentTime.value,id)&&!confirm("Já existe atendimento exatamente neste horário. Salvar mesmo assim?"))return;const obj={id,clientId:appointmentClient.value,methodId:appointmentMethod.value,methodNameSnapshot:(old&&old.methodId!==appointmentMethod.value)?(method?.name||"Método"):(old?.methodNameSnapshot||method?.name||"Método"),date:appointmentDate.value,time:appointmentTime.value,baseCents:C.toCents(appointmentBaseValue.value),extras:extras(),paymentMethod:appointmentPaymentMethod.value,financialStatus:appointmentFinancialStatus.value,expectedReceiveDate:appointmentFinancialStatus.value==="a_receber"?appointmentExpectedDate.value:"",receivedDate:appointmentFinancialStatus.value==="recebido"?appointmentReceivedDate.value:"",summary:appointmentSummary.value.trim(),deletedAt:null,createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString(),schemaVersion:2};const errs=C.validateAppointment(obj);if(errs.length){alert("Não foi possível salvar: "+errs.join(", "));return}await put(STORES.appointments,obj);localStorage.removeItem("cartonath-appointment-draft");appointmentDialog.close();await refresh();showToast("Atendimento salvo.")}
